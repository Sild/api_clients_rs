...
impl V1Dex {
    ...
    pub async fn exec(&self, req: &V1DexReq) -> ApiClientsResult<V1DexRsp> {
        let rsp = match req {
            ...
            V1DexReq::AssetsQuery(params) => V1DexRsp::AssetsQuery(self.executor.exec_post("assets/query", &params).await?),
            ...
            V1DexReq::Routers(params) => V1DexRsp::Routers(self.executor.exec_get_with_params("routers", &params).await?),
            V1DexReq::Router(addr) => V1DexRsp::Router(self.executor.exec_get(&format!("routers/{addr}")).await?),
            V1DexReq::SwapSimulate(params) => V1DexRsp::SwapSimulate(self.executor.exec_post("swap/simulate", params).await?),
            _ => unimplemented!(),
        };
        Ok(rsp)
    }
