...
async fn test_get_pools() -> Result<()> {
    let client = init_env();
    let req = V2Req::Pools;
    let V2Rsp::Pools(rsp) = client.v2_exec(&req).await? else { panic!("V2Rsp::Pools expected") };
    assert_ne!(rsp, vec![]);
    Ok(())
}
...
async fn test_get_pools_lite() -> Result<()> {
    ...
    let V2Rsp::PoolsLite(rsp) = client.v2_exec(&req).await? else { panic!("V2Rsp::PoolsLite expected") };
    assert_ne!(rsp, vec![]);
    Ok(())
}
...
async fn test_get_pool_trades() -> Result<()> {
    ...
    let V2Rsp::PoolTrades(rsp) = client.v2_exec(&req).await? else { panic!("V2Rsp::PoolTrades expected") };
    assert_ne!(rsp, vec![]);
    Ok(())
}
